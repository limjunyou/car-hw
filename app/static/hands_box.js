...
function onResults(results) {
  ...
  canvasCtx.fillText("+", 175, 100);
  canvasCtx.fillText("-", 225, 100);
  canvasCtx.fillText("1", 175, 280);
  canvasCtx.fillText("0", 225, 280);
  canvasCtx.restore();
  
  let src = cv.imread('output_canvas');
  if (cx > 50 && cx < 250 && cy > 50 && cy<250 && counter==1) 
  {
    age = age + 1;
    if (age>99){
      age = 99;
    }
	  cv.rectangle(src, new cv.Point(50, 50), new cv.Point(250, 150), [0, 255, 0, 255], -1); //RGBA - A for alpha
  }else
  {
	  cv.rectangle(src, new cv.Point(50, 50), new cv.Point(250, 150), [255, 0, 0, 255], 4); //RGBA - A for alpha
  }
  
  if (cx > 225 && cx < 300 && cy > 100 && cy<200 && counter==1)
  {
    age = age - 1;
    if (age<0){
      age = 0
